...
Sync.once = function (...)
	...
	log('sched', 'DETAIL', 'created Sync.once %s from %s with signal descriptor %s', sync,f,...)
	return sync
end
...
Task.new = function (...)
	...
	task.args={}
	log('sched', 'INFO', 'Task.new created %s from function %s by %s', task, f,sched.running)
	return task
end
...
function T:kill()
	...
	if self.status~='dead' then
		log('sched', 'INFO', "Task.kill killing %s from %s",self,sched.running)
		signal(self,'dying')
		self:finalize()
		self.status='dead'
		signal(self,'dead')
...
function T:resume()
	...
	log('sched', 'DETAIL', "Resuming %s", obj)
...
function sched.reset()
	...
	name='scheduler',
	kill=function(obj)
		log('sched', 'INFO', "killing %s from %s", obj,sched.running)
