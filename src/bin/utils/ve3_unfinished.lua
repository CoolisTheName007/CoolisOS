...
function Vec:add(vec)
	util.forEach(vec,function(k,v) self[k]=self[k]+v end)
end
...
class.Mem()
function Mem:__call(...)
	local types=table.concat(util.map({...},ofType),'|')
	if self[types] then
		return self[types](...)
	else
		error(string.format('Expected args of types %s, got %s',table.concat(util.keys(self),' OR '),types),2)
	end
end
...
function Vec3:cross(vec)
	self.x=(self.y*vec.z)-(vec.y*self.z)
	self.y=(self.x*vec.z)-(vex.x*self.z)
	self.z=(self.x*vec.y)-(vec.x*self.y)
end
...
class.Mem()
function Mem:__call(...)
	local id=table.concat(util.map({...},typeof),'|')
	if id then
		return self[id](...)
	else
		error(string.format('type of args %s is not one of %s',id,table.concat(util.keys(self),';')))
	end
end
...
function Vec3.add['number|number|number'](self,x,y,z)
	self.x =self.x+x
    self.y =self.y+y
    self.z =self.z+z
end
Vec3.addScalars=Vec3.add['number|number|number']

function Vec3.add:Vec3(v)
	return self:addScalars(v.x,v.y,v.z)
end
Vec3.addVec3=Vec3.add.Vec3

function Vec3.add:number(n)
	return self:addScalars(n,n,n)
end
Vec3.addScalar=Vec3.add.number


Vect3.subtract=Mem()
function Vec3.subtract['number|number|number'](self,x,y,z)
	self.x =self.x-x
    self.y =self.y-y
    self.z =self.z-z
end
Vec3.subtract3n=Vec3.subtract['number|number|number']

function Vec3.subtract:Vec3(v)
	return self:subtract3n(v.x,v.y,v.z)
end
Vec3.subtractVec3=Vec3.subtract.Vec3

function Vec3.subtract:number(n)
	return self:subtract3n(n,n,n)
end
Vec3.subtractScalar=Vec3.subtract.number
